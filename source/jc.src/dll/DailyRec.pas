...
var
  fmDailyRec: TfmDailyRec;

implementation

uses RepFwoDm, RpPdDaly,RpPdStat;

{$R *.DFM}

procedure TfmDailyRec.btnRightClick(Sender: TObject);
begin
...
procedure TfmDailyRec.btnRightAllClick(Sender: TObject);
begin
...
procedure TfmDailyRec.btnLeftClick(Sender: TObject);
begin
...
procedure TfmDailyRec.btnLeftAllClick(Sender: TObject);
begin
  if lbGetData.Items.Count >0  then
  repeat
    lbChoseData.Items.Add(lbGetData.Items.Strings[lbGetData.itemindex] );
...
procedure TfmDailyRec.btnExitClick(Sender: TObject);
begin
  close;
end;

procedure TfmDailyRec.FormCreate(Sender: TObject);
begin
...
procedure TfmDailyRec.ButtonLockCal;
begin
...
procedure TfmDailyRec.QRExpr2Print(sender: TObject; var Value: String);
begin
  if Value <>'' then
    Value := CurrencyString + Value;
end;

procedure TfmDailyRec.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  with quSeleCat do
    if Active then
      Close;
...
procedure TfmDailyRec.FormShow(Sender: TObject);
begin
  with quSeleCat do
    if Not Active then
      Open;
...
procedure TfmDailyRec.btnPrintClick(Sender: TObject);
var i : Integer;
begin
  if lbGetData.Items.Count >0 then
  begin
    ...
    begin
      ...
      Application.CreateForm(TfmRpPdDaly,fmRpPdDaly);
      with fmRpPdDaly do
      begin
      ...
      begin
        ...
        begin
          ...
          begin
            if Not Active then
              Open;
            qrlStatusDescr.Caption := '';
            if recordcount > 0 then
            begin
              repeat
                if qrlStatusDescr.Caption <> '' then
                  qrlStatusDescr.Caption := qrlStatusDescr.Caption
                    + ', ' + Trim(quProdStatusPRODUCTION_STATUS.AsString)
                    + '-' + Trim(quProdStatusDESCRIPTION.AsString)
                else
                  qrlStatusDescr.Caption := '狀態說明 :'+ Trim(quProdStatusPRODUCTION_STATUS.AsString)
                    + '-' + Trim(quProdStatusDESCRIPTION.AsString);
                Next;
              until eof;
            end;
...
procedure TfmDailyRec.dblcDevelopChange(Sender: TObject);
begin
  lbChoseData.Clear;
  with quCostCat do
  begin
