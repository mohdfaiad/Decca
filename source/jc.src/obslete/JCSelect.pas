...
var
  fmJCSelect: TfmJCSelect;

implementation
   uses JCdatadm,JobMastr,JCmain;
{$R *.DFM}

procedure TfmJCSelect.dgrJob_fileDblClick(Sender: TObject);
begin
  try
...
procedure TfmJCSelect.FormCreate(Sender: TObject);
begin
...
procedure TfmJCSelect.sbExitClick(Sender: TObject);
begin
...
procedure TfmJCSelect.sbPSClick(Sender: TObject);
begin
  printSetup.Execute;
end;

procedure TfmJCSelect.btnOKClick(Sender: TObject);
var
    jobno1,
    jobno2,
    subno,
...
procedure TfmJCSelect.btnCancelClick(Sender: TObject);
begin
  with dmJCdata do
  begin
    with  quJob_FileView do
...
procedure TfmJCSelect.edJobNo1KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmJCSelect.edJobNo2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmJCSelect.edJobNo1Change(Sender: TObject);
begin
  if ((Length(edJobNo1.Text)
    = dmJCdata.quJCcontrolJOB_NO_1_LEN.AsInteger)
     and edJobNo2.Visible = True) then
